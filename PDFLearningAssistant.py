...
启动后访问 http://localhost:7860 即可使用这个智能学习助手。
"""

# ============================================================================
# audioop-lts 兼容垫片（Python 3.13 移除了 audioop 标准库，gradio/pydub 需要它）
# ============================================================================
import sys
import audioop  # 来自 audioop-lts 包
...
class PDFLearningAssistant:
    ...
    def load_document(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            # 使用RAG工具处理PDF：MarkItDown转换 → 智能分块 → 向量化
            # chunk_size=4000（原1000）：减少分块数→减少本地CPU嵌入调用次数，加载提速约4倍
            # 注意：Qdrant memory 模式数据在进程内，重启服务后需重新上传文档
    ...
    def ask(self, question: str, use_advanced_search: bool = True) -> str:
        ...
        # 使用RAG高级检索获取答案（MQE 多查询扩展 + HyDE 假设文档嵌入）
...
def create_gradio_ui():
    ...
    def load_pdf(pdf_file) -> str:
        ...
        # Gradio 6.x: File(type="filepath") 返回字符串路径（NamedString）
        # Gradio 4.x: 返回带 .name 属性的临时文件对象 → 两种都要兼容
    ...
    def chat(message: str, history: List) -> Tuple[str, List]:
        ...
        注意：gradio 6.x Chatbot 强制使用 messages 格式（list[MessageDict]），
        不再支持旧的 tuples 格式（list[list[str, str]]）。这里输出标准
        {'role': 'user/assistant', 'content': ...} 字典列表。
        """
        if assistant_state["assistant"] is None:
    ...
    # 创建 Gradio 界面（gradio 6.0 起 theme 移到 launch()）
    with gr.Blocks(title="智能文档问答助手") as demo:
        ...
        - - 加载 PDF 文档并构建知识库
        - - 智能问答（基于 RAG + MQE/HyDE 高级检索）
        - - 学习笔记记录（基于 Memory 四类记忆）
        - - 学习历程回顾
        - - 学习报告生成
        """)
        ...
        with gr.Tab("🏠 开始使用"):
            with gr.Row():
                user_id_input = gr.Textbox(
                    label="用户ID",
                    placeholder="输入你的用户ID（可选，默认为 web_user）",
                    value="web_user"
                )
...
def main():
    ...
    print("=" * 60)
    print("正在启动 Web 界面...")
    print("启动后访问 http://localhost:7860 即可使用这个智能学习助手\n")
